...
# --- IMAGE BASE64 DATA (INSERT YOUR STRINGS HERE) ---
# --- IMAGE BASE64 DATA ---
...
def create_html_report(df):
    ...
    # Use the globally available Base64 strings
    university_heading_b64 = UNIVERSITY_HEADING_B64
    heading_b64 = HEADING_B64
    
    # Select only the relevant columns for the final report presentation
    report_cols = [
        "Rank", 
        "Event Name", 
        "Participant Name", 
...
def get_google_sheet_client():
    ...
    try:
        ...
        # NOTE: This assumes the private key is pasted using the multiline triple-quote format.
...
def load_sheet_data(sheet_name):
    ...
    try:
        ...
        # FIX: Ensure Event_ID is treated as an integer for reliable filtering
        if sheet_name == PARTICIPANT_SHEET_NAME and 'Event_ID' in df.columns:
...
def score_submission_form(judge_name, event_name, event_date, c_names, participant_names, is_new, selected_participant_name=None, initial_scores=[0.0, 0.0, 0.0, 0.0]):
    """Renders the score submission form, pre-filled if modifying an old score."""
    
    max_score = MAX_SCORE
    step = SCORE_STEP
    
    # Logic for participant selection in the New Entry tab
    if is_new:
        selected_participant_name = st.selectbox("Select Participant to Score", [""] + participant_names, key='new_participant_select')
    else:
        # In modify tab, participant is already selected, just display name
        st.markdown(f"**Modifying Score for:** **{selected_participant_name}**")
    
    if not selected_participant_name:
        return
        
    form_key = f"score_form_{'new' if is_new else 'modify'}_{selected_participant_name}"
    
    with st.form(form_key, clear_on_submit=is_new):
        st.markdown(f"**Adjust Marks (Max {max_score:.0f} per criterion):**")
        
        # --- SLIDER INPUTS ---
        # Note: We use hidden labels and display the labels/names in a separate column for better layout
        ...
        if st.form_submit_button(submit_label):
            ...
            if write_score_to_sheet(score_data):
                ...
                st.success(f"✅ {action} for **{selected_participant_name}** ({total:.1f}) recorded in Google Sheet! (Old score is overwritten by averaging.)")
...
def judge_score_page():
    ...
    event_name = st.session_state.assigned_event_name
    ...
    # FIX: Participant filtering is now guaranteed to work because Event_ID is an integer
    event_participants_df = participants_df[participants_df['Event_ID'] == e_id]

    if event_participants_df.empty:
        st.warning(f"No participants found for event '{event_name}' in the metadata sheet.")
        return
        
    participant_names = event_participants_df['P_Name'].tolist()
    ...
    with tab_new:
        ...
        score_submission_form(judge_name, event_name, event_date, c_names, participant_names, is_new=True)
    ...
    with tab_modify:
        ...
        # Get the latest score for each participant by this judge
        latest_scores_df = judge_scores_df.sort_values(by='Date & Time Entered', ascending=False).drop_duplicates(subset=['Participant Name'])
        
        st.dataframe(latest_scores_df[[
            'Date & Time Entered', 'Participant Name', 
            'Criterion1_Marks', 'Criterion2_Marks', 'Criterion3_Marks', 'Criterion4_Marks', 'Total'
        ]], use_container_width=True, hide_index=True)

        st.divider()
        st.markdown("### Modify a Previous Score")
        
        # Dropdown to select participant whose score needs modifying
        modify_participant_names = latest_scores_df['Participant Name'].tolist()
        
        selected_participant_modify = st.selectbox(
            "Select Participant to Modify", 
            [""] + modify_participant_names, 
            key='modify_participant_select'
        )

        if selected_participant_modify:
            # Find the latest submission row for the selected participant
            past_row = latest_scores_df[latest_scores_df['Participant Name'] == selected_participant_modify].iloc[0]
            ...
            score_submission_form(
                ...
                participant_names, 
                is_new=False,
                selected_participant_name=selected_participant_modify,
                initial_scores=[
...
def read_and_process_scores():
    ...
    Returns: (aggregated_df, results_df) or None if no scores found.
    """
    ...
    score_cols = ['Criterion1_Marks', 'Criterion2_Marks', 'Criterion3_Marks', 'Criterion4_Marks', 'Total']
    for col in score_cols:
        if col in results_df.columns:
             results_df[col] = pd.to_numeric(results_df[col], errors='coerce').fillna(0)
             
    # 2. Calculate Aggregates (Average Scores)
    # NOTE: Grouping by Judge and Participant implicitly handles score modification, 
    # as the average across all judges will reflect the judge's latest submission.
    aggregated_results = results_df.groupby(["Event Name", "Participant Name", "Judge Name"]).agg(
        Criterion1_Avg=('Criterion1_Marks', 'last'), # Use 'last' for judge's latest score
        Criterion2_Avg=('Criterion2_Marks', 'last'),
        Criterion3_Avg=('Criterion3_Marks', 'last'),
        Criterion4_Avg=('Criterion4_Marks', 'last'),
        Total_Submitted=('Total', 'last')
    ).reset_index()

    # Now average across judges for FINAL score
    final_aggregated = aggregated_results.groupby(["Event Name", "Participant Name"]).agg(
        Criterion1_Avg=('Criterion1_Avg', 'mean'),
        Criterion2_Avg=('Criterion2_Avg', 'mean'),
        Criterion3_Avg=('Criterion3_Avg', 'mean'),
        Criterion4_Avg=('Criterion4_Avg', 'mean'),
        Final_Total=('Total_Submitted', 'mean'),
        Judges_Count=('Judge Name', 'nunique')
    ).reset_index()
    
    # 3. Merge with Participant Metadata (Joining on Participant Name)
    
    if 'P_Name' in participants_df.columns:
        participant_details = participants_df.rename(columns={'P_Name': 'Participant Name'})
    else:
        participant_details = participants_df

    # Select only the required columns from participant data
    participant_details = participant_details[['Participant Name', 'Email Id', 'Mobile No', 'Organization Name']].drop_duplicates(subset=['Participant Name'])
    
    # Perform Merge (Joining on the renamed 'Participant Name')
    final_df = pd.merge(
        final_aggregated,
        participant_details,
        on='Participant Name',
        how='left'
    ).drop_duplicates(subset=['Event Name', 'Participant Name']) 

    # 4. Rename columns for final display
    final_df.columns = [
        "Event Name", "Participant Name", "Criterion 1 (Avg)", "Criterion 2 (Avg)", 
        "Criterion 3 (Avg)", "Criterion 4 (Avg)", "Final Total Score", "Judges Count",
        "Email Id", "Mobile No", "Organization Name"
    ]
...
def results_page():
    ...
    # Image display removed to bypass Base64 error
    if HEADING_B64:
        st.markdown(f'<img src="data:image/png;base64,{HEADING_B64}" class="img-fluid" style="width: 100%;">', unsafe_allow_html=True)
    else:
        st.warning("Event heading image not loaded (Placeholder Active).")
    ...
    aggregated_results, raw_scores_df = processed_tuple # Correctly unpacks results_df to raw_scores_df
    
    if aggregated_results.empty:
        st.info("No scores found in the Google Sheet (after processing).")
        return
    ...
    if max_all_option == "Winners":
        if not filtered_agg_df.empty:
            ...
            if selected_event == "All Events":
                ...
                def get_top_two(df):
                    return df.sort_values(by="Final Total Score", ascending=False).head(2)
                ...
                final_display_df = filtered_agg_df.sort_values(by="Final Total Score", ascending=False).head(2).reset_index(drop=True)

            # Add Rank column for Winners list
            final_display_df['Rank'] = final_display_df.groupby('Event Name')['Final Total Score'].rank(ascending=False, method='min').astype(int)
        ...
        final_display_df = filtered_agg_df.sort_values(
            by=["Event Name", "Final Total Score"], 
            ascending=[True, False] 
        )
        final_display_df['Rank'] = final_display_df.groupby('Event Name')['Final Total Score'].rank(ascending=False, method='min').astype(int)


    # 3. Display Results - Selectively display columns
    
    # Columns to display in the final table
    display_cols = ["Rank", "Participant Name", "Final Total Score", "Email Id", "Mobile No", "Organization Name", "Event Name", "Judges Count"]
    
    # Filter final_display_df to show only the required columns, maintaining order
    display_df = final_display_df[[col for col in display_cols if col in final_display_df.columns]]
    
    st.markdown("### Processed Results (Average Score of All Judges)")
...
# NOTE: Removed st.title() here as the image is used instead
...
if st.session_state.logged_in:
    ...
    if st.session_state.role == "Admin":
        # ADMIN: Only displays the Results Page (Readonly Dashboard)
        st.markdown(f"## Welcome, {st.session_state.username}!")
        results_page()
    
    elif st.session_state.role == "Judge":
        # JUDGE: Only displays the Score Entry page 
        st.markdown(f"## Welcome, Judge {st.session_state.username}!")
        
        # Display ONLY the Score Entry tab
...
try:
    ...
    print(records)
    print("Google Sheet connection successful. Current records in Score Sheet:")
except Exception as e:
