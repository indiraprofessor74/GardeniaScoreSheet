...
def create_html_report(df):
    """Generates an A4-sized HTML report with custom headers and styled table."""
    if df.empty:
...
# ----------------------------------------------------------------------------------------------------
## Utility Functions (Google Sheets Connection)
# ----------------------------------------------------------------------------------------------------
...
def login_user_gsheet():
    ...
    st.title("🎤 Event Score Submission System")
...
def judge_score_page():
    ...
    st.markdown("## 📝 Judge Score Entry")
    
    # FIX: Get current assigned event details from session state
    ...
    selected_participant_name = st.selectbox("Select Participant to Score", participant_names)
    ...
    st.markdown(f"### Entering Score for: **{selected_participant_name}**")
    
    with st.form("score_submission_form", clear_on_submit=True):
        st.markdown(f"**Adjust Marks (Max {MAX_SCORE:.0f} per criterion):**")
        
        # --- SLIDER INPUTS ON ONE LINE EACH ---
        ...
        if st.form_submit_button("Submit Score to Google Sheet 💾"):
            # Capture the current date and time
            submission_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

            # Data list MUST match your Google Sheet column order
            score_data = [
                judge_name, 
                event_name, 
                event_date, 
            ...
            if write_score_to_sheet(score_data):
                st.success(f"✅ Score for **{selected_participant_name}** ({total:.1f}) recorded in Google Sheet!")
...
def read_and_process_scores():
    ...
    aggregated_results = results_df.groupby(["Event Name", "Participant Name"]).agg(
        Criterion1_Avg=('Criterion1_Marks', 'mean'),
        Criterion2_Avg=('Criterion2_Marks', 'mean'),
        Criterion3_Avg=('Criterion3_Marks', 'mean'),
        Criterion4_Avg=('Criterion4_Marks', 'mean'),
        Final_Total=('Total', 'mean'),
        Judges_Count=('Judge Name', 'nunique')
    ).reset_index()
    
    # 3. Merge with Participant Metadata (Joining on Participant Name)
    
    # Prepare Participant Metadata for Merge
    ...
    participant_details = participant_details[['Participant Name', 'Email Id', 'Mobile No', 'Organization Name']]
    ...
    # Correct return: aggregated DataFrame and raw results DataFrame
...
st.set_page_config("Score Management Panel (Pure Google Sheet)", layout="wide")
...
if st.session_state.logged_in:
    ...
    if st.session_state.role == "Admin":
        ...
        tab1 = st.tabs(["📝 Judge Score Entry"]) 
...
try:
    ...
    print(f"Final check error is : {e}")